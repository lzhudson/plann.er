    ...
    <div className="h-16 bg-zinc-900 px-4 rounded flex items-center shadow-shape gap-3">
      <button type="button" onClick={openGuestsModal} className="flex items-center gap-2 flex-1 text-left">
        ...
        {emailsToInvite.length > 0 ? (
          <span className="text-zinc-100 text-lg flx-1">{emailsToInvite.length} pessoa(s) convidada(s)</span>
        ) : (
          <span className="text-zinc-400 text-lg flx-1">Quem estára na viagem?</span>
        )}
      </button>
